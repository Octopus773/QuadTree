...
TEST_CASE("isInsideConvexPolygon functional tests", "[QuadTree][Collisions][isInsideConvexPolygon]")
{
	std::vector<std::pair<double, double>> polygon1 = {{0, 0}, {10, 0}, {10, 10}, {0, 10}};
	std::pair<double, double> p = {20, 20};
	CHECK(!Collisions::isInsideConvexPolygon(polygon1, p));
	...
	p = {3, 3};
	CHECK(Collisions::isInsideConvexPolygon(polygon2, p));
	...
	p = {-1,10};
	CHECK(!Collisions::isInsideConvexPolygon(polygon3, p));
}